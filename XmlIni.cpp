...
XmlIni::XmlIni( LPWSTR inifileName )
{
	...
	if( loadOkay == false )
	{
		//		auto decl = new XMLDeclaration( "1.0", "", "" );
...
std::wstring
XmlIni::GetIniStringW( LPSTR section, LPSTR key, LPSTR defaultValue )
{
	std::wstring ret;

	LPSTR ans = GetIniString( section, key, defaultValue );
	WCHAR buf[ MAX_PATH ];
	ShiftJisToUtf8( buf, ans );
	ret.assign( buf );
